    ...
    <table class="table">
      ...
      <tbody class="table-group-divider">
        <tr v-for="(user, index) in users" :key="user.id">
          ...
          <td>
            <button class="btn ms-2" :class="user.status == 0 ? 'btn-success' : 'btn-danger'"
...
export default {
  setup() {
    ...
    //Viết hàm lọc user khác với id hiện tại đang đăng Nhập
    const userFilter = () => {
      users.value = users.value.filter(user => user.id !== isLogin.value.id);
    };
    onMounted(async () => {
      await getUsers();
      userFilter();
